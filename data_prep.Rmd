...
```{r}
df <- read_csv('HIV_AIDS_Diagnoses_by_Neighborhood,_Sex,_and_Race_Ethnicity_20251026.csv')
...
Based on the [source](https://www.nyc.gov/assets/doh/downloads/pdf/ah/surveillance2020-tables-all.pdf), I’ll keep the one with the closest number.
...
```{r}
df_bkp <- df
write_csv(df, 'cleaned_hiv_data.csv')
dim(df_bkp)
```
...
```{r}
write_csv(aggs, 'aggregated_hiv_data.csv')

write_csv(df, 'no_aggs_data.csv')
```
